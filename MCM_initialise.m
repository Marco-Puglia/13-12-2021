...
% Trait dimensions
eco_pars.nsize      = 20;                                                  % total number of plankton size classes
eco_pars.ntroph     = 20;                                                  % total number of plankton trophic classes

% quota
eco_pars.muinf_a    = +4.70;   % /d                                        % Ward et al. AmNat 2017
eco_pars.muinf_b    = -0.26;                                               % Ward et al. AmNat 2017

eco_pars.Vmax_a     = +0.024;  % mmolN/cell/d                               % Ward et al. AmNat 2017
eco_pars.Vmax_b     = +1.10;         

eco_pars.Qmin_a     = +0.032;  % mmolN/cell                                 % Ward et al. AmNat 2017
...
eco_pars.refuge     = -10;                                                % grazing refuge parameter (negative)
                                                                           
eco_pars.pp_opt     = +1000;                                               % optimum pred:prey ratio
eco_pars.pp_sig     = +2;                                                  % width grazing kernel
eco_pars.lambda     = +0.7;                                                % grazing efficiency (sloppy feeding)
...
% (NOT USED)
% eco_pars.Dremin = 0.01;                                                  % Remineraliszation
...
%%%%%%%%%%%%%MAKE GRIDS WITH DIAMTERS, VOLUMES and trophic matrix to indicate phototrophic and phagotrophic organisms
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%INFLUENCE VOLUME ON GR
eco_pars.muinf   = eco_pars.muinf_a .* eco_pars.V .^ eco_pars.muinf_b;       %Mu infinite: The maximum potential metabolic rate
eco_pars.Vmax    = eco_pars.Vmax_a  .* eco_pars.V .^ eco_pars.Vmax_b;        %Maximum nutrient uptake rate
eco_pars.Qmin    = eco_pars.Qmin_a  .* eco_pars.V .^ eco_pars.Qmin_b;        %basal cellular nutrient requirement
% Edwards et al. L&O 2012
eco_pars.alpha   = eco_pars.alpha_a .* eco_pars.V .^ eco_pars.alpha_b;       % m3 of H2O cleared of N by a single cell / time, when N is scarce
eco_pars.g       = eco_pars.g_a     .* eco_pars.V .^ eco_pars.g_b;           % m3 of H2O cleared of N (inside P) by a single cell / time, when N is scarce

eco_pars.allometric.muinf=[eco_pars.muinf_a eco_pars.muinf_b];     % Growth rate according to size
eco_pars.allometric.Qmin =[eco_pars.Qmin_a  eco_pars.Qmin_b ];     % Quota for each cell to stay same size
eco_pars.allometric.Vmax =[eco_pars.Vmax_a  eco_pars.Vmax_b ];     % Maximum nutrient uptake rate
eco_pars.allometric.alpha=[eco_pars.alpha_a eco_pars.alpha_b];     % m3 H2O cleared of N by a single cell / time, when N is scarce
eco_pars.allometric.g    =[eco_pars.g_a     eco_pars.g_b    ];     % Zooplankton equivalent of alpha.
eco_pars=rmfield(eco_pars,{'muinf_a','muinf_b','Qmin_a','Qmin_b','Vmax_a','Vmax_b','alpha_a','alpha_b','g_a','g_b'});

eco_pars.mumax=eco_pars.muinf .* eco_pars.Vmax ./ (eco_pars.muinf .* eco_pars.Qmin + eco_pars.Vmax ); % d^{-1}
...
if eco_pars.ntroph>1
    ...
    eco_pars.mumax = eco_pars.mumax .*    eco_pars.trophic .^eco_pars.tau; %trophic for phototrophy
    eco_pars.g     = eco_pars.g     .* (1-eco_pars.trophic).^eco_pars.tau; %1-trophic for phagotrophy
end


%% NONNEGATIVE OPTION SEEMS TO VIOLATE MASS CONSERVATION (NOT USED)
